...
bool GenerateDesktopFile(
	...
	const auto sourceFile = u":/misc/xyz.nextalone.nagram.desktop.desktop"_q;
	...
	if (!Core::UpdaterDisabled()) {
		...
		if (!Core::Launcher::Instance().customWorkingDir()) {
			QFile::remove(u"%1xyz.nextalone.nagram.desktop._%2.desktop"_q.arg(
				targetPath,
				md5Hash));
		...
		QFile::remove(u"%1xyz.nextalone.nagram.desktop.%2.desktop"_q.arg(
			targetPath,
			md5Hash));
	}
...
bool GenerateServiceFile(bool silent = false) {
	...
	if (!Core::UpdaterDisabled()
		...
		QFile::remove(u"%1xyz.nextalone.nagram.desktop._%2.service"_q.arg(
			targetPath,
			md5Hash));
	}
...
QString psAppDataPath() {
	// Previously we used ~/.NagramDesktop, so look there first.
	// If we find data there, we should still use it.
	auto home = QDir::homePath();
	if (!home.isEmpty()) {
		auto oldPath = home + u"/.NagramDesktop/"_q;
...
void start() {
	QGuiApplication::setDesktopFileName([&] {
		...
		if (!Core::UpdaterDisabled()) {
			return u"xyz.nextalone.nagram.desktop._%1"_q.arg(
				Core::Launcher::Instance().instanceHash().constData());
		}

		return u"xyz.nextalone.nagram.desktop"_q;
	}());
